import React, { useEffect } from 'react';
import { makeStyles } from '@material-ui/core/styles';
import {
    Card,
    CardHeader,
    CardContent,
    CardActions,
    Divider,
    Grid,
    Button,
    TextField
} from '@material-ui/core';
import { useHistory } from "react-router-dom";
import {useDispatch, useSelector} from "react-redux";
import {loadFormulaItem, changeFormula, updateFormula, addNewFormula} from "redux/actions/formulaAction";


interface IFormulaItemProps {
    className: string,
    match: any
}

const useStyles = makeStyles((theme) => ({
    root: {
        padding: theme.spacing(4)
    }
}));

const FormulaItem = (props: IFormulaItemProps) => {
    const history = useHistory();
    const classes = useStyles();
    const dispatch = useDispatch();
    const paramId = props.match.params.id;
    const formulaId = paramId === 'new' ? 0 :parseInt(paramId);
    const { className, ...rest } = props;

    const formulaItem  = useSelector((state: any)=> state.formula.formulaItem);
    // const isLoading = useSelector((state: any) => state.formula.isLoading);
    // const errorValue = useSelector((state: any) => state.formula.error);
    const hasError = useSelector((state: any) => state.formula.hasError)

    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const product = {...formulaItem, [event.target.name]: event.target.value};
        dispatch(changeFormula(product))
    };

    /**
     * Сохранить изменения
     * @param event
     */
    const saveHandler = (event: React.MouseEvent) => {
        if (formulaId === 0) {
            dispatch(addNewFormula(formulaItem));
        } else {
            dispatch(updateFormula(formulaItem));
        }
        console.log(hasError);
        if (!hasError) history.push('/catalogs/formula');
    };

    useEffect( ()=> {
            if (formulaId !== 0) dispatch(loadFormulaItem(formulaId));
        }, [dispatch]
    );

    return (
        <div className={classes.root}>
            <Card {...rest} className={className}>
                <form autoComplete="off" noValidate>
                    <CardHeader
                        subheader=""
                        title="Рецептура"
                    />
                    <Divider />
                    <CardContent>
                        <Grid container spacing={3}>
                            <Grid
                                item
                                xs={12}
                            >
                                <TextField
                                    fullWidth
                                    label="Наименование продукции"
                                    margin="dense"
                                    name="name"
                                    onChange={handleChange}
                                    required
                                    value={formulaItem.product.name}
                                    variant="outlined"
                                />
                            </Grid>
                        </Grid>

                    </CardContent>
                    <Divider />
                    <CardActions>
                        <Button
                            color="primary"
                            variant="contained"
                            onClick={saveHandler}
                        >
                            Сохранить
                        </Button>
                        <Button
                            color="default"
                            variant="contained"
                            onClick={(event => history.push('/catalogs/product'))}
                        >
                            Отменить
                        </Button>
                    </CardActions>
                </form>
            </Card>
        </div>
    );
};

export default FormulaItem;
